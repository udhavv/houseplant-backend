...
// 🎁 Optional: welcome bonus for new users (set to 0 to disable)
...
export const register = async (req, res) => {
  try {
    ...
    // Create user with default plant + welcome bonus
    const user = await prisma.user.create({
      data: {
        email: email.toLowerCase(),
        username: username.toLowerCase(),
        ...
        emailVerificationToken,
        emailVerificationExpires,
        coins: SIGNUP_BONUS_COINS, // 🎁 welcome bonus
        plants: {
          create: {} // auto-create default plant
        }
      },
      include: {
        plants: true // need plant id for milestone
      }
    })

    // Log welcome bonus + create initial plant milestone (atomic)
    const sideEffects = []
...
export const resendVerification = async (req, res) => {
  try {
    // Support both middleware conventions
    ...
    // 🛡️ Rate limit: block resend if a token was issued less than 60s ago
    if (
      user.emailVerificationExpires &&
      user.emailVerificationExpires.getTime() - Date.now() > 23 * 60 * 60 * 1000 + 60 * 1000
    ) {
