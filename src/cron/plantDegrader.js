...
const degradePlantHealth = async () => {
  console.log('🌱 Running plant health degradation...')
  ...
  try {
    ...
    for (const plant of plants) {
      ...
      if (hoursSinceWatered > 6) {
        ...
        console.log(`🌿 Plant "${plant.name}" health dropped to ${newHealth}`)

        // Send warning email if health < 30% and still alive
        if (newHealth < 30 && newHealth > 0) {
          await sendWiltingEmail(plant.user.email, plant.name)
          console.log(`📧 Warning email sent for "${plant.name}"`)
        }
...
// Create the cron job with scheduled: false to control startup
const job = cron.schedule('0 */6 * * *', degradePlantHealth, {
...
// Export the job for control elsewhere
export default job